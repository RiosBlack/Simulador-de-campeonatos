...
import { listChampionshipsForUser } from "@/_services/championship.service";
...
export default async function DashboardPage() {
  ...
  const championships = await listChampionshipsForUser(session.user.id);

  const myTeams = await prisma.championshipTeam.findMany({
  ...
  return (
    <PageEntrance>
      ...
      <section className="mb-8">
        <h2 className="mb-4 text-lg font-semibold">Minhas Copas</h2>
        {championships.length === 0 ? (
          <Card>
            <p className="text-muted">
              Você ainda não participa de nenhuma copa.
            </p>
          </Card>
        ) : (
